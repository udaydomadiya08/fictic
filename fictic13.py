...
def detect_beats(music_path, fps=100):
    """
    Basic beat detection using energy peaks. Returns list of beat times (seconds).
    For better results, use a proper beat detection library (librosa, madmom, etc.).
    """
    audio = AudioFileClip(music_path)
    arr = audio.to_soundarray(fps=44100)
    arr_mono = arr.mean(axis=1)  # convert stereo → mono
    ...
    # simple peak picking
    from scipy.signal import find_peaks
    peaks, _ = find_peaks(energies, distance=20)
...
def create_best_scene_file(first_clip_path, out_path="best_scene.mp4"):
    ...
    clip = VideoFileClip(first_clip_path).subclip(start, end)
...
# ============================================================
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4"):
...
# ============================================================
def merge_final_video(best_path, fillers_path, music_path, out_path):
   

    # ---------------------------
    # Load main clips
    # ---------------------------
    best = VideoFileClip(best_path)
    fillers = VideoFileClip(fillers_path)

    # Combine video parts
    final = concatenate_videoclips([best, fillers], method="compose")

  
    seg_start, seg_end = find_continuous_segment(
        music_path,
        best.duration,
        fillers.duration
    )

    music_audio = AudioFileClip(music_path).subclip(seg_start, seg_end)

    
    music_audio = music_audio.audio_fadeout(1.5)



    # ---------------------------
    # 🎵 MUSIC DUCKING LOGIC
    # best scene → 0.03 volume
    # fillers → 0.08 volume
    # ---------------------------
    duck1 = music_audio.subclip(0, best.duration).volumex(0.05)
    duck2 = music_audio.subclip(best.duration, final.duration).volumex(0.2)

    # final shaped music
    ducked_music = concatenate_audioclips([duck1, duck2])

    # Attach ducked music
    final = final.set_audio(CompositeAudioClip([final.audio, ducked_music]))
    ...
    # ---------------------------
    txt = (
        TextClip("Uday_editZ", fontsize=40, color="white", font="Arial", method="label")
        .set_position("center")
        .set_opacity(0.4)
...
def main(args):
    ...
    for i, v in enumerate(video_inputs):
        ...
        out_path = f"final_video_{i}_{timestamp}.mp4"
