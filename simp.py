...
def choose_transition_advanced(prev, nxt, beat_strength, visual_score):
    ...
    # if beat_strength > 0.7 and visual_score > 20:
    #     trans_func = random.choice(high_energy)
    #     return trans_func(prev, nxt, dur=0.2)  # slightly longer for impact
    # elif beat_strength > 0.4 or visual_score > 15:
    #     trans_func = random.choice(medium_energy)
    #     return trans_func(prev, nxt, dur=0.2)  # medium smooth transition
    # elif visual_score > 10:
    #     trans_func = random.choice(subtle_energy)
    #     return trans_func(prev, nxt, dur=0.2)  # gentle and natural
    # else:
    return None
...
def stitching_with_audio_visual_cues(seq_clips, beat_data, visual_scores):
    """
    Stitch clips together with audio/visual cues.
    Automatically unpacks (clip, key) tuples if needed.
    """
    # If list contains tuples, extract the first element (the actual clip)
    clean = [c[0] if isinstance(c, tuple) else c for c in seq_clips]
    return concatenate_videoclips(clean, method="compose")
