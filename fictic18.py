...
def detect_beats(music_path, fps=44100, frame_size=2048, hop_size=512, min_peak_distance=20):
    """
    Basic beat detection using energy peaks.
    Returns a list of beat times in seconds.
    """
    audio = AudioFileClip(music_path)
    arr_mono = to_mono_safe(audio, fps=fps)
    ...
    # pick peaks with minimum distance
    peaks, _ = find_peaks(energies, distance=min_peak_distance)
...
def compute_visual_scores(clips, frame_time=0.1):
    """
    Compute visual difference scores between consecutive clips.
    - clips: list of VideoFileClip objects
    - frame_time: time (in seconds) from start/end to grab frames
    Returns a list of scores (higher = more visual change)
    """
    visual_scores = []
    if len(clips) < 2:
        return [0.0]  # no difference possible
    ...
    for i in range(len(clips) - 1):
        ...
        # Resize next_gray to match prev_gray shape
...
#!/usr/bin/env python3
"""
Fillers-only pipeline (self-contained):
- Includes helpers: run_ffmpeg, detect_hw_accel, fast_export, lossless_concat, export_hq
- create_fillers_file uses fast temp exports, auto filter, and HQ final export
- merge_final_video uses HQ final export
"""
...
def detect_hw_accel():
    ...
    try:
        ...
        # extend for nvenc/vaapi/amf if needed
        return hw
    except Exception:
        return {}
...
def fast_export(input_clip_or_path, out_path, fps=60, crf=18, preset="superfast", pix_fmt="yuv420p", use_hw=True, threads=4):
    ...
    Accepts MoviePy VideoClip or a path to an input file.
    """
...
# ----------------------------
# Helper: final HQ export
# ----------------------------
def export_hq(clip, out_path, use_hw_final=False, threads=8):
    """
    Final HQ export — use only for deliverables.
    If clip is a MoviePy clip, write_videofile is used; if it's a path, ffmpeg is used.
    """
    # If input is a path string => use ffmpeg direct
    if isinstance(clip, str):
        if use_hw_final and HW:
            hw_enc = HW.get("hevc") or HW.get("h264")
            cmd = [
                "ffmpeg", "-y", "-i", clip,
                "-c:v", hw_enc,
                "-preset", "slow",
                "-b:v", "0",
                "-pix_fmt", "yuv420p",   # hw often limited to yuv420p
                "-movflags", "+faststart",
...
# ----------------------------
# TEMP fast export wrapper for compatibility with older code
# ----------------------------
def export_temp(clip, out_path):
    return fast_export(clip, out_path, fps=60, crf=18, preset="superfast", pix_fmt="yuv420p", use_hw=True, threads=4)

# ======================================================
# CREATE FILLERS FILE (HQ FINAL) - uses helper functions
# ======================================================
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4", music_path=None, history=None):
    print("🔍 Running create_fillers_file... (optimized)")
    history = history or []

    global chosen_filter

    # ---- Extract filler candidates ----
    candidates = extract_candidates(video_files)
    if not candidates:
        print("❌ No candidates found for fillers.")
        return None, history
    ...
    # ---- Stitch transitions between filler clips ----
    from transit import stitching_with_audio_visual_cues
    first_clip = seq_clips[0]
    rest_clips = seq_clips[1:]

    if rest_clips:
        stitched_rest = stitching_with_audio_visual_cues(
            rest_clips,
            beat_data[1:],
            visual_scores[1:]
        )
    ...
    # ---- Apply smart effects clip-by-clip ----
    ...
    # ---- Combine stitched filler clips ----
    fillers_clip = concatenate_videoclips(stitched, method="compose").set_fps(60)

    # ---- TEMP FAST EXPORT (use hardware if available) ----
    temp_combined_path = "temp_combined_fillers.mp4"
    export_temp(fillers_clip, temp_combined_path)

    # ---- Auto filter selection (since best scene may be removed) ----
    chosen_filter = fast_auto_choose(VideoFileClip(temp_combined_path))
    ...
    print(f"✅ FILLERS READY → {out_path} (filter: {chosen_filter})")
    return out_path, history

# =================================================
# MERGE FINAL VIDEO (FILLERS ONLY)
# =================================================
def merge_final_video(fillers_path, music_path, out_path):
    # unwrap filler path if needed
    if isinstance(fillers_path, (tuple, list)):
        fillers_path = fillers_path[0]

    fillers_clip = VideoFileClip(fillers_path)

    # ---- ONLY MUSIC IN FINAL VIDEO (NO VOCALS BECAUSE NO BEST SCENE) ----
    try:
        music_clip = AudioFileClip(music_path).volumex(0.4)
        music_clip = music_clip.subclip(0, fillers_clip.duration)
    except Exception:
        music_clip = None

    if music_clip:
        final_audio = CompositeAudioClip([music_clip])
        final = fillers_clip.set_audio(final_audio)
    else:
        final = fillers_clip
    ...
    watermark = TextClip(
        ...
        font="Arial",
        method="label"
    ).set_duration(final.duration).set_opacity(0.4).set_position("center")
    ...
    ).set_duration(5).on_color(size=(1080, 1920), color=(0,0,0), col_opacity=0)

    final = concatenate_videoclips([final, donation])

    # ---- HQ Export ----
    export_hq(final, out_path, use_hw_final=False)
    ...
    print("🎉 FINAL VIDEO (FILLERS ONLY) READY:", out_path)
    return out_path
...
def main(args):
    ...
    for i, v in enumerate(video_inputs):
        print(f"\n🎬 Producing FILLERS-ONLY video {i+1}/{num_videos}")

        history = []
        fillers_path, history = create_fillers_file(
            v["fillers"],
        ...
        # Merge fillers into final video
        os.makedirs("out", exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = f"out/final_video_{i}_{timestamp}.mp4"

        merge_final_video(fillers_path, v["music"], out_path)

        print("🎉 COMPLETED:", out_path)
...
if __name__ == "__main__":
    ...
    # ARGUMENT PARSER
    # ---------------------------
    p = argparse.ArgumentParser(description="Fictic punch-intro (FILLERS ONLY PIPELINE).")
    p.add_argument("--input_videos", required=True, help="Folder with input filler clips")
    p.add_argument("--music", required=True, help="Music file or folder")
    p.add_argument("--out", default="./out/final_edit_9x16.mp4", help="Output file path")
    p.add_argument("--whisper_model", default=WHISPER_DEFAULT, help="Whisper model (unused)")
    p.add_argument("--language", default=None, help="Whisper language code (unused)")
    args = p.parse_args()
    ...
    seconds = 1.0     # Duration of beep
    frequency = 880   # Beep frequency
    ...
    #to call: python3 fictic18.py --input_videos ./input_vid --music ./music --out "out/" --whisper_model tiny 