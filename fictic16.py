...
# CREATE BEST SCENE
# ---------------------------
def create_best_scene_file(first_clip_path, out_path="best_scene.mp4", intensity_main=0.85, history=None):
    import random
    history = history or []
    ...
    clip.write_videofile(temp_audio_video, codec="libx264", audio_codec="aac", fps=30, preset="fast", threads=4, verbose=False, logger=None)
    ...
    if os.path.exists(temp_audio_video): os.remove(temp_audio_video)
    ...
    processed_clip = clip_with_audio

    # --- FAST FILTER ---
    temp_processed = "temp_processed_scene.mp4"
    processed_clip.write_videofile(temp_processed, codec="libx264", audio_codec="aac", fps=30, preset="superfast", threads=8, logger=None)

    global chosen_filter
    chosen_filter = fast_auto_choose(processed_clip)

    temp_filtered = "temp_filtered_scene.mp4"
    temp_filtered, chosen_filter = fast_apply_filter(temp_processed, temp_filtered, filter_name=chosen_filter)
    if os.path.exists(temp_processed): os.remove(temp_processed)

    clip_with_filter = VideoFileClip(temp_filtered)
    clip_with_filter.write_videofile(out_path, codec="libx264", audio_codec="aac", fps=30, preset="superfast", threads=8,
                                     logger=None,
                                     ffmpeg_params=["-movflags", "+faststart", "-crf", "18", "-pix_fmt", "yuv420p",
                                                    "-af", "loudnorm=I=-14:TP=-1.5:LRA=11,alimiter=limit=0.85",
                                                    "-c:a", "aac"])
    if os.path.exists(temp_filtered): os.remove(temp_filtered)

    print(f"✅ FINAL: best scene saved with FAST filter '{chosen_filter}'")
...
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4", music_path=None):
    ...
    global chosen_filter  # use the same filter as best scene
    ...
    for idx, c in enumerate(stitched):
        if hasattr(c, "audio") and c.audio:
            stitched[idx] = c.volumex(0.0)

    fillers_clip = concatenate_videoclips(stitched, method="compose").set_fps(60)

    temp_combined_path = "temp_combined_fillers.mp4"
    fillers_clip.write_videofile(temp_combined_path, codec="libx264", audio_codec="aac", fps=30, preset="superfast", threads=8)

    temp_filtered = "temp_filtered_fillers.mp4"
    temp_filtered, applied_filter = fast_apply_filter(temp_combined_path, temp_filtered, filter_name=chosen_filter)

    final_clip = VideoFileClip(temp_filtered)
    final_clip.write_videofile(out_path, codec="libx264", audio_codec="aac", fps=60, preset="superfast", threads=8,
                               ffmpeg_params=["-movflags", "+faststart", "-crf", "18", "-pix_fmt", "yuv420p"])
    final_clip.close()
    if os.path.exists(temp_combined_path): os.remove(temp_combined_path)
    if os.path.exists(temp_filtered): os.remove(temp_filtered)
...
def merge_final_video(best_path, fillers_path, music_path, speech_loudness, out_path):
    ...
    # Filler music
    filler_music = AudioFileClip(music_path).subclip(0, fillers_clip.duration).volumex(0.8).set_start(best_clip.duration)

    final_audio = CompositeAudioClip([combined_best_audio, filler_music])
    final_video = final_video.set_audio(final_audio)

    # Watermark
    watermark = TextClip("Uday_editZ", fontsize=40, color="white", font="Arial", method="caption", transparent=True)\
    ...
    donation = TextClip(
        ...
        fontsize=62, color="white", font="Arial", method="caption", transparent=True, size=(1080, 1920)
    ).set_duration(5).on_color(size=(1080, 1920), color=(0,0,0), col_opacity=1).set_position("center")


    final_video = concatenate_videoclips([final_video, donation])

    final_video.write_videofile(out_path, codec="libx264", audio_codec="aac", fps=60,
                                preset="fast", threads=4, temp_audiofile="temp_audio.m4a", remove_temp=True)

    for clip in [best_clip, fillers_clip, final_video]:
        try: clip.close()
        except: pass
    if os.path.exists(temp_transition_file): os.remove(temp_transition_file)
...
def main(args):
    ...
    from datetime import datetime
    import os
    from smtaud import compute_speech_loudness
    ...
    for i, v in enumerate(video_inputs):
        ...
        # Create best scene
        best_scene_file, _ = create_best_scene_file(
            merged_path,
        ...
        os.remove(temp_audio_path)  # cleanup temp audio
...
if __name__ == "__main__":
    ...
    #to call: python3 fictic16.py --first_clip_folder "firstclip" --input_videos ./input_vid --music ./music --out "out/" --whisper_model tiny 