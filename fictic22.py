...
def create_best_scene_file(first_clip_path, out_path="best_scene.mp4", intensity_main=0.85, history=None, music_path=None):
    ...
    beat_data = compute_beat_data([processed_clip], beat_times, total_duration=processed_clip.duration, tolerance=0.4)
...
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4", music_path=None, history=None):
    ...
    # --------------------------------------------------
    # Beat logic for reverse transitions
    # --------------------------------------------------
    beat_times = detect_beats(music_path) if music_path else []
    beat_tolerance = 0.40  # 200 ms window
    ...
    while i < len(seq_clips)-1:
        ...
        if should_reverse(i):
            ...
            new_c2 = concatenate_videoclips(
                [rev_c1.crossfadeout(0.1), c2.crossfadein(0.1)],
                method="compose"
            )
        ...
        # Normal flash + person-static transition
        print(f"🧍‍♂️ PERSON-STATIC for clip {i} → {i+1}")
        new_c2 = flash_transition(c1, c2, dur=0.3)
        stitched_rest.append(new_c2)
        i += 1

    # --------------------------------------------------
    # Build the final stitched list
    # --------------------------------------------------
    if len(stitched_rest) == 0:
        print("❌ stitched_rest is empty. Nothing to stitch.")
        return None

    stitched = [seq_clips[0]] + stitched_rest

   
    # No visual / beat effects — only mute audio
    # --------------------------------------------------
    for idx in range(len(stitched)):
        c = stitched[idx]

        # mute if existing audio
...
if __name__ == "__main__":
    ...
    #to call: python3 fictic22.py --first_clip_folder "firstclip" --input_videos ./input_vid --music ./music --out "out/" --whisper_model tiny 