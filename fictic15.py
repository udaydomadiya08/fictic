...
#only scenes maker jointer no fillers
...
def create_full_video(first_clip_path: str, music_path: str, out_path: str):
    """
    COMPLETE processing pipeline:
    1) Crop + audio separate + clean vocals
    2) Inject speech
    3) AI Enhance (ficint1)
    4) Music looping + ducking
    5) Watermark
    6) Donation Screen
    7) Final Export
    """
    ...
    from moviepy.editor import (
        VideoFileClip, AudioFileClip, CompositeAudioClip, TextClip,
    ...
    # ------------------------------------------------------------------
    # STEP 1 — Load clip & extract clean vocals
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # STEP 2 — 9:16 CROP + Inject Speech
    # ------------------------------------------------------------------
    resized_clip = smart_crop_clip(clip)
    cleaned_audio = AudioFileClip(clean_vocals)

    processed_clip = resized_clip.set_audio(cleaned_audio.volumex(3.0))

    # ------------------------------------------------------------------
    # STEP 3 — AI PROCESSING (ficint1)
    # ------------------------------------------------------------------
    base_best_clip = process_video_return_clip(processed_clip)

    # ------------------------------------------------------------------
    # STEP 4 — MUSIC LOOPING + DUCKING (🔥 FIXED)
    # ------------------------------------------------------------------
    video_duration = base_best_clip.duration
    scene_audio = base_best_clip.audio

    # LOOP MUSIC AUTOMATICALLY TO FULL LENGTH
    looped_music = (
    ...
    if scene_audio:
        combined_audio = CompositeAudioClip([scene_audio, looped_music])
    else:
        combined_audio = looped_music

    final_video = base_best_clip.set_audio(combined_audio)
...
def main(args):
    ...
    # ---------------------------------------------------------
    # 1️⃣ Collect all inputs for every video
    # ---------------------------------------------------------
    video_inputs = []
    ...
    # ---------------------------------------------------------
    # 2️⃣ PROCESS ALL VIDEOS (NO FILLERS)
    # ---------------------------------------------------------
    for i, v in enumerate(video_inputs):
        print(f"\n🎬 Producing video {i+1}/{num_videos}")

        # -----------------------------------------------------
        # Step A — Merge multiple first clips (if more than 1)
        # -----------------------------------------------------
        ...
        # -----------------------------------------------------
        # Step B — FINAL PROCESSING PIPELINE (best scene + ducking + watermark)
        # -----------------------------------------------------
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        final_out = f"out/final_video_{i}_{timestamp}.mp4"

        # Full pipeline in ONE FUNCTION
        create_full_video(
            first_clip_path=merged_path,
            music_path=v["music"],
            out_path=final_out
        )

        print("🎉 FINAL VIDEO READY:", final_out)
...
if __name__ == "__main__":
    ...
    #to call: python3 fictic15.py --first_clip_folder "firstclip" --input_videos ./input_vid --music ./music --out "out/" --whisper_model tiny 