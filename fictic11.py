...
def main(args):
    ...
    if not video_files or not music_files:
        print("No input videos or music files found.")
        return

    # --- First clip selection ---
    first_clip_files = list_files(args.first_clip_folder, extensions=[".mp4", ".mov", ".mkv"])
    if not first_clip_files:
        print("No first clip video found.")
        return

    print("Select the first clip video by index:")
    for i, f in enumerate(first_clip_files):
        print(f"[{i}] {os.path.basename(f)}")
    while True:
        try:
            selection = int(input("Enter the index of the first clip video: "))
            if 0 <= selection < len(first_clip_files):
                break
        except ValueError:
            pass
        print("Invalid index, try again.")

    first_clip_path = first_clip_files[selection]
    ...
    if best_scene:
        ...
        print(f"Best scene found: {start:.2f}s → {end:.2f}s")

        # --- Extract only the best scene ---
        full_clip = VideoFileClip(first_clip_path).subclip(start, end)

        def ensure_size(c):
            # target 9:16 vertical ratio
            target_w = OUT_W       # e.g. 1080
            target_h = OUT_H       # e.g. 1920
            ...
            if input_ratio > target_ratio:
                ...
                x1 = (w - new_w) // 2
                return c.crop(x1=x1, y1=0, x2=x1 + new_w, y2=h)
            else:
                # video is taller → crop top + bottom
                new_h = int(w / target_ratio)
                y1 = (h - new_h) // 2
                return c.crop(x1=0, y1=y1, x2=w, y2=y1 + new_h)


        resized_clip = ensure_size(full_clip)

        from ficint1 import process_video_return_clip

        # --- Process and get final clip ---
        processed_clip = process_video_return_clip(resized_clip)

        # --- SAVE BEST SCENE FILE ---
        best_scene_path = "/Users/uday/Downloads/fictic/best_scene_final.mp4"
        print(f"Saving processed best scene -> {best_scene_path}")
        ...
        print("Saved best scene file successfully!")

        # --- Load saved file as a clip for later prepend ---
        best_clip = VideoFileClip(best_scene_path)

        # Safe to close originals
        full_clip.close()
        resized_clip.close()
...
# 
    ...
    video_selections, music_selections = [], []

    # --- Interactive selection of inputs ---
    ...
    for vid_num in range(num_videos):
        ...
        if not selected_videos or not selected_music:
            print("Skipping video due to missing inputs.")
            continue
        ...
        print(f"Music segment: {start:.2f}s → {end:.2f}s")
        ...
        print(f"Music duration processed: {target_duration:.2f}s")

        # --- Extract visual candidates & fillers ---
        candidates = extract_candidates(selected_videos)
        if not candidates:
            print("No visual candidates; skipping video.")
            continue

        beats = detect_beats(selected_music)
        beat_gaps = [max(0.45, min(2.6, g)) for g in np.diff(beats)] if beats is not None and len(beats) > 1 else None

        seq_clips = build_fillers(selected_videos, candidates, target_duration, used_key=None, beat_gaps=beat_gaps)
        if best_clip:
            seq_clips = [best_clip] + seq_clips

        beat_times = detect_beats(selected_music)
        visual_scores = compute_visual_scores(seq_clips)
        beat_data = compute_beat_data(seq_clips, beat_times, total_duration=music_audio.duration)

        from transit import stitching_with_audio_visual_cues
        final = stitching_with_audio_visual_cues(seq_clips, beat_data, visual_scores)

        # --- Fillers if duration short ---
        if final.duration < target_duration - 0.02:
            extra = build_fillers(selected_videos, candidates, target_duration - final.duration, used_key=None, beat_gaps=beat_gaps)
            if extra:
        ...
        # --- Final adjustments ---
        final = final.subclip(0, target_duration) if final.duration > target_duration else final
        final = final.set_fps(FPS).set_duration(target_duration)
        final = final.set_audio(music_audio.set_duration(final.duration))

        # --- Watermark ---
        txt_clip = TextClip("Uday_editZ", fontsize=30, color="white", font="Arial", method="label")\
            .set_opacity(0.4).set_position("center").set_duration(final.duration)
        final_with_text = CompositeVideoClip([final, txt_clip])

        # --- Donation clip ---
        donation_clip = TextClip(
            "If you like my videos\nplease like, comment\nand subscribe!\n\nThank you for your support:)\n\nUday_editZ",
            fontsize=70, color="white", font="Arial", method="label", size=(1080, 1920)
        ).set_duration(5).set_position("center").on_color(color=(0, 0, 0), col_opacity=1.0)
...
# You should run this only if this is your main module
