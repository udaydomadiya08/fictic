...
def compute_bar_energies(music_path, beats_per_bar=4):
    ...
    if len(beats) < beats_per_bar:
        ...
        if arr.ndim == 2:
            arr = arr.mean(axis=1)
        energies = [np.sqrt(np.mean(arr**2))]
    ...
    for bar in bars:
        ...
        arr = segment.to_soundarray(fps=22050)
        if arr.ndim == 2:
            arr = arr.mean(axis=1)
...
def to_mono_safe(audio_clip, fps=44100):
    ...
    try:
        ...
        duration = audio_clip.duration if audio_clip.duration else 1.0
...
def detect_beats(
    ...
    This version is robust for stereo/mono audio and short clips.
    
    Args:
        music_path (str): Path to the audio/music file.
        fps (int): Sampling rate to read audio.
        frame_size (int): Number of samples per frame for energy computation.
        hop_size (int): Hop size between frames.
        cluster_gap_sec (float): Merge beats closer than this within a cluster.
        min_transition_gap_sec (float): Minimum gap between final beats for transitions.
    
    Returns:
        final_beats (list of float): List of seconds where beats occur.
    """
    # --- Load audio safely ---
    audio = AudioFileClip(music_path)
    
    # Use iter_frames to avoid MoviePy to_soundarray issues
    arr = np.array([frame for frame in audio.iter_frames(fps=fps, dtype='float32')])
    ...
    # --- Merge very close beats in clusters ---
    merged = [beat_times[0]]
...
def find_continuous_segment(music_path, best_dur, filler_dur, beats_per_bar=4):
    total_required = best_dur + filler_dur

    bars, energies = compute_bar_energies(music_path)

    best_score = -1e9
    best_start = 0
    best_end = 0

    # Convert duration → bars approx
    avg_bar_dur = np.mean([b[-1] - b[0] for b in bars])
    total_bars_needed = max(1, int(total_required // avg_bar_dur))

    if total_bars_needed >= len(bars):
        total_bars_needed = len(bars) - 1

    for i in range(len(bars) - total_bars_needed):
        window_bars = energies[i:i+total_bars_needed]
        mid = int(len(window_bars) * (best_dur / total_required))

        # first part = rising (buildup)
        first = window_bars[:mid]
        second = window_bars[mid:]

        if len(first) < 2 or len(second) < 2:
            continue

        slope = np.polyfit(np.arange(len(first)), first, 1)[0]
        second_energy = np.mean(second)

        # score
        score = slope * 0.6 + second_energy * 0.4

        if score > best_score:
            best_score = score
            best_start = i
            best_end = i + total_bars_needed

    segment_start = bars[best_start][0]
    segment_end = bars[best_end-1][-1]
...
def compute_beat_data(clips, beat_times, total_duration=None, tolerance=0.2):
    """
    For each transition (between clips), check if it's close to a merged beat time.
    Returns 1.0 for close-to-beat, 0.0 otherwise.
    - tolerance: seconds within which a transition is considered close to a beat
    """
    beat_data = []
    cumulative_time = 0
    for i in range(len(clips) - 1):
...
# ---- Helper: ffmpeg-run and HW detection ----
def run_ffmpeg(cmd):
...
def detect_hw_accel():
    ...
    try:
        out = subprocess.run(["ffmpeg", "-hide_banner", "-encoders"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        ...
        # NVIDIA/AMF/VAAPI could be added similarly if you run on those boxes
        return hw
    except Exception:
        return {}
...
# ---- Helper: fast intermediate exporter ----
def fast_export(input_clip_or_path, out_path, fps=30, crf=30, preset="superfast", pix_fmt="yuv420p", use_hw=True, threads=8):
    """
    Export a clip quickly for intermediate steps.
    - input_clip_or_path: MoviePy VideoClip or path to file
    - out_path: output file path
    - use_hw: allow usage of hardware encoder if detected
    """
    ...
    # Determine hardware encoder (prefer H264 hw)
    hw_enc = None
    if use_hw and HW:
        hw_enc = HW.get("h264") or HW.get("hevc")

    if isinstance(input_clip_or_path, str):
        # direct ffmpeg call is faster than going through MoviePy
        enc = hw_enc or "libx264"
        cmd = [
        ...
        # MoviePy clip path
        codec_to_use = hw_enc or "libx264"
...
# ---- Helper: lossless concat (ffmpeg demuxer) ----
...
# ---- Improved final HQ exporter ----
def export_hq(clip, out_path, threads=8):
    """
    Final high-quality export using software libx264 only.
    No hardware encoding, no audio filters that cause broken pipe.
    """
    if isinstance(clip, str):
        # If clip is a file path, use ffmpeg directly
        cmd = [
            "ffmpeg", "-y", "-i", clip,
            "-c:v", "libx264",
            "-preset", "superfast",      # slower but high quality
            "-crf", "28",           # adjust quality (lower = better)
            "-pix_fmt", "yuv420p",  # widely compatible
            "-movflags", "+faststart",
            "-c:a", "aac",
...
def create_best_scene_file(first_clip_path, out_path="best_scene.mp4", intensity_main=0.85, history=None, music_path=None):
    history = history or []
    clip = VideoFileClip(first_clip_path)
    temp_audio_video = "temp_scene_for_audio.mp4"

    try:
        # Fast intermediate export for audio extraction (use lower res if you want even faster)
        fast_export(clip, temp_audio_video, fps=30, crf=30, preset="superfast", pix_fmt="yuv420p", use_hw=True)
        try:
    ...
    # Compute visual score & beat strength
    visual_score = compute_visual_scores([processed_clip])[0]
    beat_times = detect_beats(music_path) if music_path else []
    beat_data = compute_beat_data([processed_clip], beat_times, total_duration=processed_clip.duration, tolerance=0.2)
    ...
    # Apply filter (fast_apply_filter should be able to accept VideoClip or path)
    ...
    # FINAL: export HQ for best scene
    # If filtered_clip is a path string (depends on your fast_apply_filter) handle both
    try:
        if isinstance(filtered_clip, str):
            # filtered_clip already written to disk -> use export_hq on path (ffmpeg)
            export_hq(filtered_clip, out_path)
        else:
            # MoviePy clip
            export_hq(filtered_clip, out_path)
    finally:
        # cleanup
        try:
            if not isinstance(filtered_clip, str):
                filtered_clip.close()
        except Exception:
            pass
        try:
...
def blur_clip(clip, ksize=25):
    """
    Apply Gaussian blur to a MoviePy clip.
    ksize: must be odd
    """
    if ksize % 2 == 0:
        ksize += 1
...
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4", music_path=None, history=None):
    print("🔍 Running create_fillers_file...")
    history = history or []
    global chosen_filter  # reuse same filter
    ...
    # -------------------------------------------
    # Beat Analysis for Reverse-Transition Logic
    # -------------------------------------------
    beat_times = detect_beats(music_path) if music_path else []
    beat_tolerance = 0.20  # 200ms allowed difference

    def should_reverse(idx):
        if len(beat_times) < idx + 3:
            return False
        a = beat_times[idx+1] - beat_times[idx]
        b = beat_times[idx+2] - beat_times[idx+1]
        return abs(a - b) <= beat_tolerance
    ...
    # -------------------------------------------
    # Build transitions with REVERSE + PERSON-STATIC
    # -------------------------------------------
    i = 0
    while i < len(seq_clips)-1:
        c1 = seq_clips[i]
        c2 = seq_clips[i+1]

        if random.random() < 0.8 and should_reverse(i):
            print(f"🔁 Replacing clip {i+1} with reverse of clip {i}")
            rev_c1 = replace_with_reverse_safe(c1, c2, should_reverse=True)
            new_c2 = concatenate_videoclips([rev_c1.crossfadeout(0.1), c2.crossfadein(0.1)], method="compose")
            stitched_rest.append(new_c2)
            i += 2  # skip next clip because it's replaced by reverse
            continue

        # Otherwise: normal PERSON-STATIC transition
        print(f"🧍‍♂️ PERSON-STATIC for clip {i} → {i+1}")
        dummy_clip = blur_clip(c1, ksize=25).set_duration(c1.duration).set_fps(c1.fps)
    ...
    # -------------------------------------------
    # Visual + Beat Strength Analysis
    # -------------------------------------------
    visual_scores = compute_visual_scores(seq_clips)
    beat_data = compute_beat_data(seq_clips, beat_times, total_duration=target_duration, tolerance=0.2)
    ...
    for idx, c in enumerate(stitched):
        ...
        c, history, _ = apply_best_effect(c, visual_score=vs, beat_strength=bs, history=history)
        stitched[idx] = c

    fillers_clip = concatenate_videoclips(stitched, method="compose").set_fps(30)

    # -------------------------
    # Fast intermediate export
    # -------------------------
...
def merge_final_video(best_path, fillers_path, music_path, speech_loudness, out_path):
    # unwrap tuples
    ...
    # create a temp transitioned video (your existing make_batch_or_single should write this)
    ...
    # Ensure filler_music does not exceed duration bounds
    try:
        filler_music = AudioFileClip(music_path).subclip(best_clip.duration, best_clip.duration + fillers_clip.duration).volumex(0.2).set_start(best_clip.duration)
    except Exception:
        filler_music = AudioFileClip(music_path).subclip(0, min(5, best_clip.duration)).volumex(0.2).set_start(best_clip.duration)

    final_audio = CompositeAudioClip([combined_best_audio, filler_music])
    final_video = final_video.set_audio(final_audio)

    # watermark
    watermark = TextClip("Uday_editZ", fontsize=40, color="white", font="Arial", method="label") \
    ...
    donation = TextClip(
        ...
        color=(0,0,0),
        col_opacity=0
    ).set_duration(5).set_position("center").set_fps(final_video.fps)


    final_video = concatenate_videoclips([final_video, donation])

    # Export final merged video with HQ exporter (software libx264 recommended for color fidelity).
    export_hq(final_video, out_path)
...
# ------------------------
# MAIN (updated to use fast_export & lossless concat)
# ------------------------
def main(args):
    # args should include: first_clip_folder, music, input_videos (folders or paths)
    # expects helper functions: list_files, choose_first_clips, choose_single_file_interactively, choose_files_interactively
    try:
    ...
    for i, v in enumerate(video_inputs):
        ...
        # Merge first clips if multiple: prefer lossless_concat when possible
        if len(v["first_clips"]) > 1:
            # verify same codec/format/resolution/fps if you want to use lossless_concat
            merged_path = f"merged_first_clips_{i}.mp4"
            try:
                lossless_concat(v["first_clips"], merged_path)
                merged_path_exists = True
            except Exception:
                # fallback to re-encoding quickly
                clips = [VideoFileClip(f) for f in v["first_clips"]]
                concatenated_clip = concatenate_videoclips(clips, method="compose")
                fast_export(concatenated_clip, merged_path, fps=30, crf=30, preset="superfast", pix_fmt="yuv420p", use_hw=True)
        ...
        # Create best scene (final HQ export inside function)
        history = []
        best_scene_result = create_best_scene_file(
            merged_path,
            out_path=f"best_scene_{i}.mp4",
            history=history,
            music_path=v["music"]
        )
        best_scene_file, history = best_scene_result

        # Create fillers with same filter & effects (final HQ export inside function)
        fillers_result = create_fillers_file(
            v["fillers"],
            target_duration=6.0,
        ...
        fillers_file, history = fillers_result

        # Compute speech loudness safely
        ...
        # Merge final video with ducking, watermark & donation screen
        os.makedirs("out", exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = f"out/final_video_{i}_{timestamp}.mp4"

        # use_hw_final=False -> highest color fidelity; set True to speed up final on M1 (tradeoff)
        merge_final_video(best_scene_file, fillers_file, v["music"], speech_loudness, out_path)
...
if __name__ == "__main__":
    ...
    #to call: python3 fictic17.py --first_clip_folder "firstclip" --input_videos ./input_vid --music ./music --out "out/" --whisper_model tiny 