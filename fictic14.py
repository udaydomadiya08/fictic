...
def smart_crop_clip(moviepy_clip):
    ...
    for frame in moviepy_clip.iter_frames(fps=moviepy_clip.fps, dtype='uint8'):
        # frame is RGB from MoviePy → convert to BGR for OpenCV
        frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        crop_frame, prev_box = smart_full_crop(frame_bgr, prev_box, history)
        frames.append(cv2.cvtColor(crop_frame, cv2.COLOR_BGR2RGB))  # back to RGB

    # Make new clip
    new_clip = ImageSequenceClip(frames, fps=moviepy_clip.fps)
    return new_clip

def create_best_scene_file(first_clip_path, out_path="best_scene.mp4"):
    # best_scene = find_best_scene(first_clip_path)
    # if best_scene is None:
    #     print("❌ No best scene found")
    #     return None

    # start, end = best_scene
    # print(f"✔ Best Scene: {start:.2f}s → {end:.2f}s")

    # 1️⃣ Extract best-scene video section
    ...
    #         x1 = (w - new_w) // 2
    #         c = c.crop(x1=x1, y1=0, x2=x1 + new_w, y2=h)
    #     else:
    #         new_h = int(w / target_ratio)
    #         y1 = (h - new_h) // 2
    #         c = c.crop(x1=0, y1=y1, x2=w, y2=y1 + new_h)

    #     return c.resize((target_w, target_h))

    resized_clip = smart_crop_clip(clip)

    # 6️⃣ Replace audio → USE ONLY VOCALS
    audio_clean = AudioFileClip(clean_vocals)
    final_clip = resized_clip.set_audio(audio_clean.volumex(3.0))  # Boost speech

    from ficint1 import process_video_return_clip

    # --- Process and get final clip ---
    final_clip = process_video_return_clip(final_clip)

    # 7️⃣ Apply professional audio chain (normalize + limiter)
    print("🎚 Normalizing + limiting audio…")
    ...
    final_clip.write_videofile(
        ...
        ffmpeg_params=[
            ...
            # FORCE RE-ENCODE (Fix broken pipe)
            "-c:v", "libx264",
            "-c:a", "aac",

            "-af",
            "loudnorm=I=-14:TP=-1.5:LRA=11,alimiter=limit=0.85"
        ]
    )
    ...
    print("✅ FINAL: best scene saved with MUSIC REMOVED + dialogue cleaned")
    return out_path
...
# ============================================================
# 2) CREATE FILLERS AS SEPARATE FILE
# ============================================================
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4", music_path=None):
    print("🔍 Running create_fillers_file...")

    # -------------------------------------
    # 1. Extract filler candidates
    # -------------------------------------
    ...
    used_key = None
    fillers = build_fillers(video_files, candidates, target_duration, used_key)

    # -------------------------------------
    # 2. ENSURE fillers IS A LIST
    # -------------------------------------
    if fillers is None:
    ...
    if not isinstance(fillers, (list, tuple)):
        print("ℹ️ fillers is a SINGLE CLIP → wrapping inside list")
        fillers = [fillers]

    seq_clips = fillers  # safe list

    # -------------------------------------
    # 3. Detect beats & compute visual cue data
    # -------------------------------------
    beat_times = detect_beats(music_path)
    visual_scores = compute_visual_scores(seq_clips)
    beat_data = compute_beat_data(
        seq_clips,
        beat_times,
    ...
    # -------------------------------------
    # 4. TRANSITION STITCHING (may return single clip!)
    # -------------------------------------
    # -------------------------------------
    # 4. TRANSITION STITCHING (may return single clip!)
    # -------------------------------------
    ...
    # Only skip transition for first clip
    # Only skip transition for first clip
    first_clip = seq_clips[0]
    rest_clips = seq_clips[1:]

    # Apply stitching to rest of clips normally
    if rest_clips:
        stitched_rest = stitching_with_audio_visual_cues(rest_clips, beat_data[1:], visual_scores[1:])
    ...
    # Combine first clip (no transition) + stitched rest
    stitched = [first_clip] + stitched_rest



    # ENSURE stitched is always a list
    if not isinstance(stitched, (list, tuple)):
        print("ℹ️ transitions returned SINGLE CLIP → wrapping inside list")
        stitched = [stitched]

    # -------------------------------------
    # 5. MUTE all filler audio
    # -------------------------------------
    processed = []
    for c in stitched:
        try:
            if hasattr(c, "audio") and c.audio is not None:
                c = c.volumex(0.0)
        except:
            pass
        processed.append(c)
    ...
    # -------------------------------------
    # 6. Concatenate FINAL FILLERS
    # -------------------------------------
    print("🎬 Concatenating filler clips...")
    fillers_clip = concatenate_videoclips(processed, method="compose")

    fillers_clip = fillers_clip.set_fps(60)

    fillers_clip.write_videofile(
        out_path,
        codec="libx264",
        audio_codec="aac",
        fps=60,
    ...
    fillers_clip.close()
    print("✅ Filler file created:", out_path)
    return out_path


# ============================================================
# 3) MERGE BEST_SCENE + FILLERS + MUSIC
# ============================================================
...
from moviepy.editor import (
    ...
    concatenate_videoclips, AudioFileClip, afx
)
from objtrans import make_batch_or_single
from typing import Optional
import os
...
def merge_final_video(best_path: str, fillers_path: str, music_path: str, out_path: str):
    """
    Merge best scene and fillers with transitions, ducked music, watermark, and donation screen.
    Handles audio properly to avoid looping and ensures fillers audio is ignored.
    """
    # ---------------------------
    # Step 1: Apply object transition
    # ---------------------------
    temp_transition_file = "temp_transition_output.mp4"

    make_batch_or_single(
        clipA_path=best_path,
        clipB_path=fillers_path,
        transition_type="slide",
        duration=0.7,
        out_path=temp_transition_file,
        batch=False
    )
    ...
    # ---------------------------
    # Step 2: Music ducking + proper audio sequencing
    # ---------------------------
    seg_start, _ = find_continuous_segment(music_path, best_clip.duration, fillers_clip.duration)

    # Original best clip audio
    best_audio = best_clip.audio if best_clip.audio else None

    # Ducked music segments
    duck1 = AudioFileClip(music_path).subclip(seg_start, seg_start + best_clip.duration).volumex(0.05)
    duck2 = AudioFileClip(music_path).subclip(seg_start + best_clip.duration,
                                              seg_start + best_clip.duration + fillers_clip.duration).volumex(0.2)
    duck2 = duck2.set_start(best_clip.duration)

    # Combine best clip audio + ducked music
    best_audio_combined = CompositeAudioClip([clip for clip in [best_audio, duck1] if clip])

    # Fillers clip: ignore its original audio, only duck2
    fillers_audio_combined = duck2

    # Concatenate audio sequentially
    from moviepy.editor import concatenate_audioclips
    final_audio = concatenate_audioclips([best_audio_combined, fillers_audio_combined])

    # Attach to final video
    ...
    # ---------------------------
    watermark = (
        TextClip("Uday_editZ", fontsize=40, color="white", font="Arial", method="caption", transparent=True)
        .set_position("center")
        .set_opacity(0.4)
    ...
    # ---------------------------
    # Step 5: Export final video safely
    # ---------------------------
    ...
    # ---------------------------
    # Step 4: Donation screen (5 sec, silent)
    # ---------------------------
    donation = (
        TextClip(
            "If you like my videos\nplease like, comment and subscribe!\n\nThank you :)\nUday_editZ",
    ...
    final_video.write_videofile(
        ...
        preset="fast",
        threads=4,
        temp_audiofile="temp_audio.m4a",  # avoids BrokenPipe errors
        remove_temp=True,
        ffmpeg_params=["-movflags", "+faststart", "-crf", "18", "-pix_fmt", "yuv420p"]
    )
...
def main(args):
    ...
    # Ask user how many videos to produce
    num_videos = input("Enter total number of videos to produce: ")
    try:
    ...
    # 2️⃣ Collect all inputs first
    video_inputs = []
    for i in range(num_videos):
        print(f"\n--- Inputs for video {i+1}/{num_videos} ---")
        
        # First clips for this video
    ...
    # 3️⃣ Now process all videos in a loop using collected inputs
    for i, v in enumerate(video_inputs):
        ...
        # Best scene
        best_scene_file = create_best_scene_file(
            merged_path,
            out_path=f"best_scene_{i}.mp4"
        )

        # Fillers
        fillers_file = create_fillers_file(
            v["fillers"],
        ...
        merge_final_video(best_scene_file, fillers_file, v["music"], out_path)
...
if __name__ == "__main__":
    ...
    #to call: python3 fictic14.py --first_clip_folder "firstclip" --input_videos ./input_vid --music ./music --out "out/" --whisper_model tiny 