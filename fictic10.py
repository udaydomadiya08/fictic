...
def main(args):
    ...
    # Ask how many final videos user wants
    ...
    # Now process videos after all inputs gathered
    for vid_num in range(num_videos):
        ...
        if not selected_videos:
            print("Skipping video due to no video selection.")
            continue
        if not selected_music:
            print("Skipping video due to no music selection.")
            continue
        ...
        # Example: args.out = "output.mp4"
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # e.g. 20251108_142530
        ...
        # Cleanup (if tmp_wdir is defined)
        try:
