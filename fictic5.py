...
def main(args):
    ...
    def choose_transition_maybe(prev, nxt, apply_prob=0.5, min_clip_dur=1.0):
        # Apply transition only if random chance met and clips long enough
    ...
    def stitching_conditionally(clips, apply_prob=0.5, min_clip_dur=0.8):
        seq = [clips[0]]
        for nxt in clips[1:]:
            prev = seq[-1]
    ...
    final = stitching_conditionally(seq_clips, apply_prob=0.4, min_clip_dur=0.8)
