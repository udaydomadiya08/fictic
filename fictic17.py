...
def detect_beats(
    ...
    for t, e in zip(beat_times[1:], beat_energies[1:]):
        if t - cluster[-1] < cluster_gap_sec:
            ...
            # pick strongest beat in cluster
...
def fast_export(input_clip_or_path, out_path, fps=60, crf=18, preset="superfast", pix_fmt="yuv420p", use_hw=True, threads=4):
    """
    ...
    if isinstance(input_clip_or_path, str):
        ...
        input_clip_or_path.write_videofile(
            ...
            fps=fps,
            preset=preset,
            ffmpeg_params=["-movflags", "+faststart", "-crf", str(crf), "-pix_fmt", pix_fmt],
            threads=threads,
            logger=None
        )
        return out_path
...
def export_hq(clip, out_path, use_hw_final=False, encoder_choice=None, threads=8):
    """
    Final HQ export — use for final deliverables only.
    - clip: MoviePy VideoClip or path to input file
    - out_path: final file
    - use_hw_final: if True and hardware encoders exist, will try to use them (faster).
    """
    if isinstance(clip, str):
        # using ffmpeg directly
        if use_hw_final and HW:
            hw_enc = encoder_choice or (HW.get("hevc") or HW.get("h264"))
            # Hardware encoders often don't support yuv422p reliably; fallback to yuv420p
            cmd = [
                "ffmpeg", "-y", "-i", clip,
                "-c:v", hw_enc,
                "-preset", "slow",
            ...
            # software libx264 with yuv422p for max chroma fidelity
            cmd = [
                "ffmpeg", "-y", "-i", clip,
                "-c:v", "libx264",
                "-preset", "veryslow",
                ...
                "-movflags", "+faststart",
                "-c:a", "aac", "-b:a", "512k", "-ar", "48000",
                "-af", "loudnorm=I=-14:TP=-1.0:LRA=11,alimiter=limit=0.95",
                "-threads", str(threads),
                out_path
            ]
        run_ffmpeg(cmd)
        ...
        # MoviePy clip -> prefer software libx264 for highest fidelity
        if use_hw_final and HW:
            hw_enc = encoder_choice or (HW.get("hevc") or HW.get("h264"))
            # MoviePy + hw encoders: set pix_fmt to yuv420p
            clip.write_videofile(
                ...
                fps=clip.fps,
                preset="slow",
                ffmpeg_params=["-movflags", "+faststart", "-b:a", "512k", "-ar", "48000"],
                threads=threads,
                logger=None
            )
        else:
...
def create_best_scene_file(first_clip_path, out_path="best_scene.mp4", intensity_main=0.85, history=None, music_path=None):
    ...
    try:
        ...
        fast_export(clip, temp_audio_video, fps=30, crf=20, preset="veryfast", pix_fmt="yuv420p", use_hw=True)
        try:
    ...
    beat_data = compute_beat_data([processed_clip], beat_times, total_duration=processed_clip.duration)
...
def create_fillers_file(video_files, target_duration, out_path="fillers.mp4", music_path=None, history=None):
    ...
    history = history or []
    global chosen_filter  # reuse the same filter
    ...
    beat_data = compute_beat_data(seq_clips, beat_times, total_duration=target_duration)

    from transit import stitching_with_audio_visual_cues
    first_clip = seq_clips[0]
    rest_clips = seq_clips[1:]
    if rest_clips:
        stitched_rest = stitching_with_audio_visual_cues(rest_clips, beat_data[1:], visual_scores[1:], bgclip=seq_clips)
        if not isinstance(stitched_rest, (list, tuple)):
            stitched_rest = [stitched_rest]
    else:
        stitched_rest = []

    stitched = [first_clip] + stitched_rest

    # Apply effects to each filler clip
    ...
    # Compose and set fps for final fillers assembly
    fillers_clip = concatenate_videoclips(stitched, method="compose").set_fps(60)

    # Fast intermediate write (use hardware if available)
    temp_combined_path = "temp_combined_fillers.mp4"
    fast_export(fillers_clip, temp_combined_path, fps=60, crf=18, preset="superfast", pix_fmt="yuv420p", use_hw=True)

    # Apply fast filter safely: assume fast_apply_filter can accept a path for input
    temp_filtered_path = "temp_filtered_fillers.mp4"
    filtered_clip, applied_filter = fast_apply_filter(temp_combined_path, out_path=temp_filtered_path, filter_name=chosen_filter)

    # Now open filtered path and export final HQ
    try:
...
def merge_final_video(best_path, fillers_path, music_path, speech_loudness, out_path, use_hw_final=False):
    # unwrap tuples
    ...
    donation = TextClip(
        ...
        fontsize=62, color="white", font="Arial", method="label", size=(1080, 1920)
    ).set_duration(5).on_color(size=(1080, 1920), color=(0,0,0), col_opacity=0).set_position("center")
    ...
    for i, v in enumerate(video_inputs):
        ...
        if len(v["first_clips"]) > 1:
            ...
            try:
                ...
                fast_export(concatenated_clip, merged_path, fps=60, crf=20, preset="veryfast", pix_fmt="yuv420p", use_hw=True)
        ...
        merge_final_video(best_scene_file, fillers_file, v["music"], speech_loudness, out_path, use_hw_final=False)
