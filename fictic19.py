...
#!/usr/bin/env python3
"""
Optimized BEST-SCENE pipeline (no fillers) with helpers included:
- run_ffmpeg, detect_hw_accel
- fast_export for intermediates (uses hw if available)
- lossless_concat for safe concat without re-encode
- export_hq for single high-quality final export (libx264, yuv422p, CRF=10)
"""
...
def detect_hw_accel():
    ...
    try:
        ...
        # add nvenc/vaapi/amf detection here if you want
        return hw
    except Exception:
        return {}
...
def fast_export(input_clip_or_path, out_path, fps=60, crf=18, preset="superfast", pix_fmt="yuv420p", use_hw=True, threads=4):
    """
    Fast export for intermediate steps. Accepts either:
     - path to existing file (string) -> runs ffmpeg directly
     - MoviePy VideoClip -> calls write_videofile (slower but works)
    """
...
def lossless_concat(file_list, out_path):
    """Concatenate files without re-encoding (requires same codec/format)."""
...
def export_hq(clip, out_path, use_hw_final=False, threads=8):
    """
    Final high-quality export. Use only for deliverables.
    - If clip is a path -> ffmpeg
    - Otherwise -> MoviePy write_videofile (libx264 ver slow)
    """
    if isinstance(clip, str):
        if use_hw_final and HW:
            ...
            cmd = [
                ...
                "-preset", "slow",
                "-b:v", "0",
                "-pix_fmt", "yuv420p",  # hw encoders often limited to 420
                "-movflags", "+faststart",
        ...
        # MoviePy clip path (in-memory clip)
        if use_hw_final and HW:
            hw_enc = HW.get("hevc") or HW.get("h264")
...
# ===============================
# CREATE BEST SCENE FILE (HQ)
# ===============================
def create_best_scene_file(first_clip_path, out_path="best_scene.mp4", intensity_main=0.85, history=None, music_path=None):
    """
    Optimized Best Scene creation:
    - fast intermediate export for audio cleaning
    - smart crop -> smart effect -> auto-filter -> HQ final export
    """
    history = history or []
    ...
    # --- AUDIO CLEANING STAGE (fast temp export for extraction) ---
    ...
    # --- Preprocess video (stabilize/resize/normalize etc.) ---
    processed_clip = process_video_return_clip(clip_with_audio)

    # --- Compute visual & beat info ---
    ...
    # --- Apply filter (fast) ---
    # If fast_apply_filter accepts a clip it may return a clip or a on-disk path
    ...
    print(f"✅ BEST SCENE CREATED with filter '{chosen_filter}'")
    history.append({"file": out_path, "filter": chosen_filter})
    return out_path, history

# =================================================
# MERGE FINAL VIDEO (NO FILLERS) – ONLY BEST SCENE
# =================================================
def merge_final_video(best_path, music_path, speech_loudness, out_path):
    """
    Merge best scene with background music (ducking), watermark and donation screen.
    Final export uses export_hq().
    """
    # unwrap tuple/list if provided
    if isinstance(best_path, (list, tuple)):
        best_path = best_path[0]

    best_clip = VideoFileClip(best_path)

    # --- Smart duck audio for best scene (returns AudioClip) ---
    ...
    final_video = best_clip.set_audio(combined_best_audio)
    ...
    # --- Donation Screen (5 sec)
    donation = TextClip(
        "If you like my videos\nplease like, comment and subscribe!\n\nThank you :)\nUday_editZ",
        fontsize=62, color="white", font="Arial",
        method="label", transparent=True, size=(1080, 1920)
    ).set_duration(5).on_color(size=(1080, 1920), color=(0,0,0), col_opacity=0)

    final_video = concatenate_videoclips([final_video, donation])

    # --- Final HQ Export (software libx264 for max color fidelity) ---
    export_hq(final_video, out_path, use_hw_final=False)
...
# =========================
# MAIN (NO FILLERS) - OPTIMIZED
# =========================
def main(args):
    """
    Main driver for BEST-SCENE-only pipeline.
    """
    try:
    ...
    for i, v in enumerate(video_inputs):
        ...
        # Merge first clips if multiple (try lossless_concat, else fast re-encode)
        ...
        # Create Best Scene (final HQ export inside)
        history = []
        ...
        # --- Compute speech loudness (temporary audio extraction) ---
        ...
        # Merge final (ducking, watermark, donation) and HQ export
        os.makedirs("out", exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = f"out/final_video_{i}_{timestamp}.mp4"
        merge_final_video(best_scene_file, v["music"], speech_loudness, out_path)
...
if __name__ == "__main__":
    ...
    #to call: python3 fictic19.py --first_clip_folder "firstclip" --music ./music --out "out/" --whisper_model tiny 