...
def main(args):
    ...
    # first_clip_folder = "/Users/uday/Downloads/fictic/first clip"  # replace with real path
    # first_clips = load_clips_from_folder(first_clip_folder)

    # Find input videos
    video_paths = find_videos(args.input_videos)
    if not video_paths:
        print("No input videos found:", args.input_videos)
        return

    # Find music file
    music_path = find_music_file(args.music)
    if not music_path:
        print("Music file not found:", args.music)
        return
    ...
    start, end = find_complete_segment(music_path, desired_duration)
    ...
    # Extract candidates from video paths
    candidates = extract_candidates(video_paths)
    if not candidates:
        print("No visual candidates found; aborting.")
        return

    # Detect beats for filler timing
    beats = detect_beats(music_path)
    beat_gaps = None
    if beats is not None and len(beats) > 1:
        gaps = np.diff(beats)
        beat_gaps = [max(0.45, min(2.6, g)) for g in gaps]
        print(f"Detected {len(beats)} beats; beat gaps prepared.")
    else:
        print("No beats detected; default filler durations used.")

    # Key for handling dialogue (if any)
    used_key = None

    # Build fill-ins
    fillers = build_fillers(video_paths, candidates, target_duration, used_key, beat_gaps=beat_gaps)

    # Build sequence from fillers
    seq_clips = fillers

    # Detect beats again for visual cue alignment
    ...
    # Stitch with cues
    from transit2 import stitching_with_audio_visual_cues
    final = stitching_with_audio_visual_cues(seq_clips, beat_data, visual_scores)

    # Append more fillers if needed
    if final.duration < target_duration - 0.02:
        print("Final shorter than music; appending more fillers...")
        extra = build_fillers(video_paths, candidates, target_duration - final.duration, used_key, beat_gaps=beat_gaps)
        if extra:
            final = concatenate_videoclips([final] + extra, method="compose")

    # Append random clips for extra length if needed
    attempts = 0
    ...
    #         font='Arial',
    #         method='label',
    #         size=(1080, 1920)  # full vertical 9:16 frame
    #     )
    #     .set_duration(5)  # 5 seconds
    #     .set_position("center")
    #     .on_color(
    ...
    #     preset="slow",
    #     threads=4,
    #     ffmpeg_params=["-movflags", "+faststart", "-crf", "15", "-pix_fmt", "yuv420p"]
    # )
    ...
    try:
        shutil.rmtree(tmp_wdir)  # ensure tmp_wdir is correctly set before calling
    except:
        pass
    print("Done ->", args.out)
