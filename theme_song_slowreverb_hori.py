...
# ---------------- CONFIG ----------------
INPUT_MP3 = "/Users/uday/Downloads/Luis Fonsi ‒ Despacito (Lyrics _ Lyric Video) ft. Daddy Yankee - Taz Network.mp3"
...
VIDEO_SIZE =(1920, 1080)   # 16:9 vertical
FPS = 30
...
# =================================================
# 🎵 SLOWED + REVERB AUDIO PROCESSING
# =================================================
def create_slowed_reverb(input_mp3, output_path, speed=0.85, reverb_amount=0.03):
    """
    Subtle reverb (just ~80ms delay) for slowed audio
    - speed < 1.0 slows down
    - reverb_amount controls echo intensity (0-1)
    """
    ...
    # 2️⃣ Subtle single-layer reverb
    delay_ms = 30  # 0.08 sec
    attenuated = slowed_audio - 6  # quieter
...
# =================================================
# =================================================
# 🚀 MAIN PIPELINE WITH SLOWED VIDEO DURATION FIX
# =================================================
if __name__ == "__main__":
    stems = separate_with_demucs(INPUT_MP3)
    merge_instrumental(stems, OUTPUT_MP3)
    
    # Extract best segment first
    # best_segment = get_best_segment(OUTPUT_MP3, SEGMENT_DURATION_MS)

    # Create slowed + reverb version
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    slowed_reverb_audio = f"best_segment_slowed_reverb_{timestamp}.mp3"
    create_slowed_reverb(OUTPUT_MP3, slowed_reverb_audio)

    # Load slowed audio to check duration
    # slowed_audio_obj = AudioSegment.from_mp3(slowed_reverb_audio)
    # if len(slowed_audio_obj) > SEGMENT_DURATION_MS:
    #     slowed_audio_obj = slowed_audio_obj[:SEGMENT_DURATION_MS]  # trim to max 58 sec
    #     slowed_audio_obj.export(slowed_reverb_audio, format="mp3", bitrate="320k")
    #     print(f"⏱ Slowed + Reverb audio trimmed to {SEGMENT_DURATION_MS/1000:.2f} sec")

    best_audio = slowed_reverb_audio  # use this for video
