...
def main(args):
    ...
    # Use first video file to find best scene clip
    ...
    # Show list with indices for user selection
    print("Select the first clip video by index:")
    for i, filepath in enumerate(first_clip_files):
        print(f"[{i}] {os.path.basename(filepath)}")

    # Read user input index safely
    while True:
        try:
    ...
    if best_scene is None:
        print("No suitable best scene found in first clip video.")
        best_clip = None
    else:
        start, end = best_scene
        print(f"Best scene found in first clip: {start:.2f}s → {end:.2f}s")
        best_clip

    from ficint1 import process_video_return_clip
    bestscene=process_video_return_clip(input_path=best_scene) 
    ...
    for vid_num in range(num_videos):
        ...
        print(f"Music segment duration processed: {target_duration:.2f}s")

        # --- Extract visual candidates ---
        candidates = extract_candidates(selected_videos)
        if not candidates:
            print("No visual candidates; skipping this video.")
            continue
        ...
        seq_clips = [bestscene] + fillers

        # Prepend best clip only in first video
        if best_clip is not None and vid_num == 0:
            seq_clips = [best_clip] + seq_clips

        # Sync with beat and visual scores
        ...
        # Append fillers if final duration short
        ...
        # Add random clips if still short
        attempts = 0
        ...
        # Trim or adjust final clip timing
        ...
        # Save output with timestamp suffix
